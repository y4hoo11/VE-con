...
> ⚠️ **実行する前に、手順4・5で起動した turtlesim と auto_drive を終了させてください。**
> 起動したままだと CPU を食い合い、Gazebo の起動が間に合わずに失敗することがあります。

### 起動中に流れる大量のメッセージの見分け方

起動の途中で、こういう行が大量に流れます。
...
| 出方 | 意味 |
| --- | --- |
| **4〜5回出て止まる** | **正常。** Gazebo がロボットを配置し終わるまでの一時的なもの |
| **0.5秒ごとに延々と続く** | **失敗。** ロボットが Gazebo 内に生成されていない（下記の対処へ） |

### ロボットが出てこない場合の対処

Gazebo は `Ctrl+C` で終了してもプロセスが生き残ることがあります。残骸が通信ポートを掴んだままだと、
次に起動した Gazebo が地図を読み込めずに固まり、ロボットが出てきません。

ホスト側（PowerShell / Git Bash）で以下を実行し、コンテナごと再起動してください。確実に残骸が消えます。
...
RViz2 には、**丸いロボットと、そこから伸びる青い線**が見えます。
青い線は LiDAR（レーザー距離センサー）が周囲を測っている様子です。

1. 上部ツールバーの **`2D Pose Estimate`** を押す
2. 地図上でロボットがいる場所をクリックし、向いている方向へドラッグして離す
   （ドラッグ中に出る大きな矢印の、根元が位置・向きが姿勢です）
   → **見えるもの**: **青い線（レーザー）が地図の壁とピタッと重なる**
   → これが**自己位置推定**。起動直後のロボットは自分がどこにいるか分かっていないので、
     最初の一回だけ人間が教えてあげる必要があります。ずれていたら何度でもやり直せます
3. 上部の **`Nav2 Goal`** を押す
